...
|`AddRaw()`|Adds a raw C++ pointer delegate. Raw pointer does not use any sort of reference (see [[C++ notes|Pointers]]), so may be unsafe to call if the object was deleted out from underneath your delegate. Be careful when calling Execute()!|
