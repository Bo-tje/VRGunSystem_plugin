...
# Week 15
This Tuesday i created the final parts of the first version of my plugin, the magwell and magazine. it went pretty well. the magwell is pretty much the same as a grabcomponent instead of you being able to grab it it grabs a compatible mag when you release it near the component and its snaps it to a socket. for the magazine base i made basicallly the same thing as the weaponbase, its a distributer that basically constructs the magazine from the data asset. It also has the option to visualise the bullets inside the mag with logic actually very similar to my blueprint version. and with these things working i actually reached my what i thaught was a very optimistic and maybe even unrealistic deadline/time estimate for completing a first full version of the plugin. and everything pretty much works like i wanted it to wich is really cool to see all my hard work pay off. 

I was inspired by Alexander's custom behavior tree in the gadot engine and began wondering, would something like that be possible in unreal engine a custom editor window. and yes it was possible and its called slate, unreals c++ language to create custom editor windows. its a low level engine language so its highly optimised and only lives in the editor so it wouldn't increase game file size.

# Week 16
On Tuesday this week i finished a lot of final improvement, feedback like sound and haptics, optimization, bug and edge case fixes and the custom editor window. it works really well the code opens a preview window so you can actually see what your building in the data asset, the data asset of course and a a custom tab that shows you the calculated stats of the weapon. i also made sure every gameplay tag input field is restricted to their respected function to prevent confusion and wrong inputs and fields where you can input parent sockets now automatically scan the parent for sockets and list them in a drop down menu preventing misspelling of socket names.
